...
function Reload(wep)
	...
	if not timer.Exists("reload" .. wep:EntIndex()) and wep.Clip ~= wep.MaxClip and wep.Amt > 0 then
		--wep:EmitSound(guninfo.ReloadSounds[math.Rand(1, #guninfo.ReloadSounds)], 85, 100, 1)
		wep:EmitSound("Weapon_AR2.Reload", 85, 100, 1)
		timer.Create(
			"reload" .. wep:EntIndex(),
			guninfo.ReloadTime,
			1,
...
function FireShot(wep)
	...
	local ply = wep:GetOwner()
	local guninfo = weapons.Get(ply.curweapon)
	wep.NextShot = CurTime() + guninfo.ShootWait*1.3
